...
#define CODELETS_ALL(name, _nbuffers, cpu_func_name, cuda_func_name, mc_func_name) \
    struct starpu_perfmodel cl_##name##_fake = {                      \
        ...
        .symbol = "fake_"#name                                          \
                                                                        ...
                                                                        \
    static struct starpu_perfmodel cl_##name##_model = {              \
        .type   = STARPU_HISTORY_BASED,                                 \
        .symbol = ""#name                                               \
    };                                                                  \
                                                                        \
    static struct starpu_perfmodel cl_##name##_mc_model = {           \
        .type   = STARPU_HISTORY_BASED,                                 \
        .symbol = "mc_"#name                                            \
    };                                                                  \
                                                                        \
    struct starpu_codelet cl_##name = {                                        \
        ...
        .model     = &cl_##name##_model                                 \
                                                                        ...
                                                                        \
    struct starpu_codelet cl_##name##_mc = {                                   \
        .where     = STARPU_CPU|STARPU_CUDA,                            \
        ...
        .model     = &cl_##name##_mc_model                              \
                                                                        ...
                                                                        \
    struct starpu_codelet cl_##name##_cpu = {                                  \
        .where     = STARPU_CPU,                                        \
        .cpu_func  = ((cpu_func_name)),                                 \
        .nbuffers  = ((_nbuffers)),                                     \
        .model     = &cl_##name##_model                                 \
...
#define CODELETS_ALL_HEADER(name)                       \
    ...
    extern struct starpu_codelet cl_##name##_mc;               \
    extern struct starpu_codelet cl_##name##_cpu;              \
    void cl_##name##_restrict_where(uint32_t where);    \
    void cl_##name##_restore_where(void);
