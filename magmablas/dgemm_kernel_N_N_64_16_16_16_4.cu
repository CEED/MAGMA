...
/*
sgemm C =alpha* A*B  + beta * C;  B is put into shared memory 
Parameters Used:
...
extern "C" __global__ void  dgemm_kernel_N_N_64_16_16_16_4
	(double *C, const double *A, const double *B, int m,int n, int k, int lda, int ldb, int ldc, double alpha, double beta){
	...
	*/
	...
	/*
	These variables guide the threads to avoid invalid memory accesses in dimension N 
...
/*
	//if(iby+ty   >=n)      { s1 = 0 ; s2 = 0*ldb ; s3 = 0*ldb ; s4 =  0*ldb; } else 
	if(iby+ty   >=n)      { s1 = 0 ; s2 = 0*ldb ; s3 = 0*ldb ; s4 =  0*ldb; B+=__mul24(0 ,ldb);} else 
        if(iby+ty+4 >=n)      { s1 = 0 ; s2 = 0*ldb ; s3 = 0*ldb ; s4 =  0*ldb; B+=__mul24(ty,ldb);} else 
        if(iby+ty+8 >=n)      { s1 = 0 ; s2 = 4*ldb ; s3 = 0*ldb ; s4 =  0*ldb; B+=__mul24(ty,ldb);} else 
        if(iby+ty+12>=n)      { s1 = 0 ; s2 = 4*ldb ; s3 = 8*ldb ; s4 =  0*ldb; B+=__mul24(ty,ldb);}
*/
	...
	*/
	if ( k!=0){
		...
		/*
			...
			If some thread enters this if() block first access to B  should be valid as K isn't divisible by blk_K
			Note that dimension N has been taken care of by s1,s2,s3,s4
			But depending upon K and thread index tx, some memory access may be still invalid, so take care of them now by setting s1,s2,s3,s4 = 0 
			B might have been advanced in the previous loop, take care of that, this is about right bottom corner. 	
		*/
...
extern "C" void
magmablas_dgemm_kernel_N_N_64_16_16_16_4( double *C, const double *A, const double *B, int m,int n, int k, int lda, int ldb, int ldc, double alpha, double beta)
{
        dim3 threads( 16, 4 );
        dim3 grid(m/64+(m%64!=0),n/16+(n%16!=0));
        dgemm_kernel_N_N_64_16_16_16_4<<< grid, threads >>> ( C, A, B, m, n, k, lda , ldb , ldc , alpha , beta ) ;
}
