...
Warning!!!! If the matrix dimension doesnt fit into block size this code will fail.
*/
extern "C" __global__ void dgemm_kernel_N_N_64_16_16_16_4_special(double *C, const double *A, const double *B, int m,int n, int k, int lda, int ldb, int ldc, double alpha, double beta) {
...
extern "C" void
magmablas_dgemm_kernel_N_N_64_16_16_16_4_special( double *C, const double *A, const double *B, int m,int n, int k, int lda, int ldb, int ldc, double alpha, double beta)
{
        dim3 threads( 16, 4 );
        dim3 grid(m/64,n/16);
        dgemm_kernel_N_N_64_16_16_16_4_special<<< grid, threads >>> ( C, A, B, m, n, k, lda , ldb , ldc , alpha , beta ) ;
}
