...
__global__ void
test_l_ssymv_special_v6_ts_fermi(int n, float alpha,  float* A, int lda, float *x, 
...
__global__ void
test_l_ssymv_special_update_v6_ts_fermi(int n, float alpha, float* A, int lda, float *x, 
...
__global__ void
test_l_ssymv_generic_v6_ts_fermi(int n, float alpha, float* A, int lda, float *x, 
...
__global__ void
test_l_ssymv_generic_update_v6_ts_fermi (int n, float alpha ,  float* A, int lda, float *x, int inx , float beta ,  float *y , int iny , float *WC, int kstan ){
  int tx = threadIdx.x ;
  int ind = (blockIdx.x)* 64 + tx ;
  float Ca = 0.f ;
  WC+= tx+(blockIdx.x)*64 + lda*blockIdx.x  ;
...
__global__ void
test_u_ssymv_generic_v6_ts_fermi (int n, float alpha ,  float* A, int lda, float *x, int inx , float beta ,  float *y , int iny , int m_full_block , int m_mod_32){
}
__global__ void
test_u_ssymv_special_v6_ts_fermi (int n, float alpha ,  float* A, int lda, float *x, int inx , float beta ,  float *y , int iny ){
}



extern "C"
void magmablas_ssymv6_fermi(char uplo, int m, float alpha, float *A, int lda, 
...
*/
    ...
    if(m % dgemv_bs == 0 ) {
       if( uplo == 'L' || uplo == 'l'){
	  test_l_ssymv_special_v6_ts_fermi <<<grid, threads>>>(m, alpha, 
                   A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
	  test_l_ssymv_special_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
       }
       else{
      ...
      if (uplo == 'L' || uplo == 'l'){
         test_l_ssymv_generic_v6_ts_fermi <<<grid, threads>>> (m, alpha, A, lda, 
                    X, incx ,beta,  Y , incy, dC_work, m_mod_64-1, kstan);
         test_l_ssymv_generic_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
      }	
      else{
         printf("Not Available Now.\n");
