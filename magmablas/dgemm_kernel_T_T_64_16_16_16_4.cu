...
extern "C" __global__ void  dgemm_kernel_T_T_64_16_16_16_4
	...
	*/
	if ( k!=0){
		...
		/*
			...
			But depending upon K and thread index tx, some memory access may be still invalid, 
			so take care of them now by setting s1,s2,s3,s4 = 0 
...
extern "C" void
magmablas_dgemm_kernel_T_T_64_16_16_16_4( double *C, const double *A, const double *B, int m,int n, int k, int lda, int ldb, int ldc, double alpha, double beta)
{
        dim3 threads( 16, 4 );
        dim3 grid(m/64+(m%64!=0),n/16+(n%16!=0));
        dgemm_kernel_T_T_64_16_16_16_4<<< grid, threads >>> ( C, A, B, m, n, k, lda , ldb , ldc , alpha , beta ) ;
}
