...
void CUDA_SAFE_CALL_US( cudaError_t err ) {
;
}
 ...
 */
extern "C"
void magmablas_dtrsm (char side, char uplo, char tran, char diag, int M, int N,  double* A, int lda, double* b, int ldb)
{
double alpha = 1.f ; 
/*  -- MAGMA (version 0.1) --
       Univ. of Tennessee, Knoxville
	...
	if (side == 'l' || side == 'L')
	{
		 ...
		 * Allocate device memory for the inversed diagonal blocks, size=m*BLOCK_SIZE 
		 */
		CUDA_SAFE_CALL_US(cudaMalloc((void**)&d_dinvA, BLOCK_SIZE*M*sizeof(double)));
		nblocks = M/BLOCK_SIZE;
		...
		/* the non-transpose case */
		{
			...
			/* the lower case */
				...
				if (BLOCK_SIZE>=M)
				{
					CUDA_SAFE_CALL_US(cudaFree(d_dinvA));
					return;
				}
	...
	{	// side=R
		 ...
		 * Allocate device memory for the inversed diagonal blocks, size=N*BLOCK_SIZE 
		 */
		CUDA_SAFE_CALL_US(cudaMalloc((void**)&d_dinvA, BLOCK_SIZE*N*sizeof(double)));
		nblocks = N/BLOCK_SIZE;
		diag_dtrtri_kernel<<<nblocks, BLOCK_SIZE>>>(uplo, diag, A, d_dinvA, lda);
