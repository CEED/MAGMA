...
#define dF(i_, j_) (dF + (i_) + (j_)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        magma_zswap_gemv_kernel
            <<< magma_ceildiv( m, BLOCK_SIZE1 ), BLOCK_SIZE1, 0, magmablasGetQueue()->cuda_stream() >>>
            ( m, rk, k, dA(0, 0), ldda, dF(k,  0), lddf, dA(0, k), dA(0,pvt));
        
        /*  Generate elementary reflector H(k). */
        magma_zscale_kernel
            <<< 1, BLOCK_SIZE, 0, magmablasGetQueue()->cuda_stream() >>>
        ...
        if (k > 0) {
            ...
            magma_zgemv_kernel
                <<< magma_ceildiv( n-k-1, BLOCK_SIZE1 ), BLOCK_SIZE1, 0, magmablasGetQueue()->cuda_stream() >>>
        ...
        if (k < n-1) {
            ...
            magma_zgemv_kernel_adjust
                <<< magma_ceildiv( n-k-1, BLOCK_SIZE2 ), BLOCK_SIZE2, 0, magmablasGetQueue()->cuda_stream() >>>
