...
__global__ void
test_l_zhemv_special_v6_ts_tesla(int n, double2 alpha,  double2* A, int lda, double2 *x, 
...
__global__ void
test_l_zhemv_special_update_v6_ts_tesla(int n, double2 alpha, double2* A, int lda, double2 *x, 
...
__global__ void
test_l_zhemv_generic_v6_ts_tesla(int n, double2 alpha, double2* A, int lda, double2 *x, 
...
__global__ void
test_l_zhemv_generic_update_v6_ts_tesla (int n, double2 alpha ,  double2* A, int lda, double2 *x, int inx , double2 beta ,  double2 *y , int iny , double2 *WC, int kstan ){
  int tx = threadIdx.x ;
  int ind = (blockIdx.x)* 64 + tx ;
  double2 Ca ={ 0.f,0.f} ;
  WC+= tx+(blockIdx.x)*64 + lda*blockIdx.x  ;
...
__global__ void
test_u_zhemv_generic_v6_ts_tesla (int n, double2 alpha ,  double2* A, int lda, double2 *x, int inx , double2 beta ,  double2 *y , int iny , int m_full_block , int m_mod_32){
}
__global__ void
test_u_zhemv_special_v6_ts_tesla (int n, double2 alpha ,  double2* A, int lda, double2 *x, int inx , double2 beta ,  double2 *y , int iny ){
}



extern "C"
void magmablas_zhemv6_tesla(char uplo, int m, double2 alpha, double2 *A, int lda, 
...
*/
    ...
    if(m % dgemv_bs == 0 ) {
       if( uplo == 'L' || uplo == 'l'){
	  test_l_zhemv_special_v6_ts_tesla <<<grid, threads>>>(m, alpha, 
                   A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
	  test_l_zhemv_special_update_v6_ts_tesla<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
       }
       else{
      ...
      if (uplo == 'L' || uplo == 'l'){
         test_l_zhemv_generic_v6_ts_tesla <<<grid, threads>>> (m, alpha, A, lda, 
                    X, incx ,beta,  Y , incy, dC_work, m_mod_64-1, kstan);
         test_l_zhemv_generic_update_v6_ts_tesla<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
      }	
      else{
         printf("Not Available Now.\n");
...
extern "C"
void  magmablas_zhemv_tesla( char uplo , int m , double2 alpha,  double2 *A , int lda , 
				double2 *X , int incx, double2 beta, double2 *Y, int incy)
{
	...
	magmablas_zhemv6_tesla(uplo, m, alpha, A, lda, X, incx, beta, Y, incy, 
                      dC_work, kstan);
