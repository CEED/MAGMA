...
#include "cuda.h"
#include "cublas.h"
#define magmablas_dsymv_fermi magmablas_dsymv 
...
__global__ void
test_l_dsymv_special_v6_ts_fermi(int n, double alpha,  double* A, int lda, double *x, 
...
__global__ void
test_l_dsymv_special_update_v6_ts_fermi(int n, double alpha, double* A, int lda, double *x, 
...
__global__ void
test_l_dsymv_generic_v6_ts_fermi(int n, double alpha, double* A, int lda, double *x, 
...
__global__ void
test_l_dsymv_generic_update_v6_ts_fermi (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny , double *WC, int kstan ){
  int tx = threadIdx.x ;
  int ind = (blockIdx.x)* 64 + tx ;
  double Ca = 0.f ;
  WC+= tx+(blockIdx.x)*64 + lda*blockIdx.x  ;
...
__global__ void
test_u_dsymv_generic_v6_ts_fermi (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny , int m_full_block , int m_mod_32){
}
__global__ void
test_u_dsymv_special_v6_ts_fermi (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny ){
}



extern "C"
void magmablas_dsymv6_fermi(char uplo, int m, double alpha, double *A, int lda, 
...
*/
    ...
    if(m % dgemv_bs == 0 ) {
       if( uplo == 'L' || uplo == 'l'){
	  test_l_dsymv_special_v6_ts_fermi <<<grid, threads>>>(m, alpha, 
                   A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
	  test_l_dsymv_special_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
       }
       else{
      ...
      if (uplo == 'L' || uplo == 'l'){
         test_l_dsymv_generic_v6_ts_fermi <<<grid, threads>>> (m, alpha, A, lda, 
                    X, incx ,beta,  Y , incy, dC_work, m_mod_64-1, kstan);
         test_l_dsymv_generic_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
      }	
      else{
         printf("Not Available Now.\n");
...
extern "C"
void  magmablas_dsymv_fermi( char uplo , int m , double alpha,  double *A , int lda , 
				double *X , int incx, double beta, double *Y, int incy)
{
	if (uplo == 'U' || uplo == 'u')
		...
		magmablas_dsymv6_fermi(uplo, m, alpha, A, lda, X, incx, beta, Y, incy, 
        	              dC_work, kstan);
