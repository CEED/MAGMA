...
__global__ void
test_l_chemv_special_v6_ts_fermi(int n, float2 alpha,  float2* A, int lda, float2 *x, 
...
__global__ void
test_l_chemv_special_update_v6_ts_fermi(int n, float2 alpha, float2* A, int lda, float2 *x, 
...
__global__ void
test_l_chemv_generic_v6_ts_fermi(int n, float2 alpha, float2* A, int lda, float2 *x, 
...
__global__ void
test_l_chemv_generic_update_v6_ts_fermi (int n, float2 alpha ,  float2* A, int lda, float2 *x, int inx , float2 beta ,  float2 *y , int iny , float2 *WC, int kstan ){
  int tx = threadIdx.x ;
  int ind = (blockIdx.x)* 64 + tx ;
  float2 Ca ={ 0.f,0.f} ;
  WC+= tx+(blockIdx.x)*64 + lda*blockIdx.x  ;
...
__global__ void
test_u_chemv_generic_v6_ts_fermi (int n, float2 alpha ,  float2* A, int lda, float2 *x, int inx , float2 beta ,  float2 *y , int iny , int m_full_block , int m_mod_32){
}
__global__ void
test_u_chemv_special_v6_ts_fermi (int n, float2 alpha ,  float2* A, int lda, float2 *x, int inx , float2 beta ,  float2 *y , int iny ){
}



extern "C"
void magmablas_chemv6_fermi(char uplo, int m, float2 alpha, float2 *A, int lda, 
...
*/
    ...
    if(m % dgemv_bs == 0 ) {
       if( uplo == 'L' || uplo == 'l'){
	  test_l_chemv_special_v6_ts_fermi <<<grid, threads>>>(m, alpha, 
                   A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
	  test_l_chemv_special_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
       }
       else{
      ...
      if (uplo == 'L' || uplo == 'l'){
         test_l_chemv_generic_v6_ts_fermi <<<grid, threads>>> (m, alpha, A, lda, 
                    X, incx ,beta,  Y , incy, dC_work, m_mod_64-1, kstan);
         test_l_chemv_generic_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
      }	
      else{
         printf("Not Available Now.\n");
...
extern "C"
void  magmablas_chemv_fermi( char uplo , int m , float2 alpha,  float2 *A , int lda , 
				float2 *X , int incx, float2 beta, float2 *Y, int incy)
{
	...
	magmablas_chemv6_fermi(uplo, m, alpha, A, lda, X, incx, beta, Y, incy, 
                      dC_work, kstan);
