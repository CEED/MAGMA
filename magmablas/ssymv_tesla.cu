...
#define magmablas_ssymv_tesla magmablas_ssymv 
#define dgemv_bs 64
#define thread_x 64
#define thread_y 4
...
__global__ void
test_l_ssymv_special_v6_ts_tesla(int n, float alpha,  float* A, int lda, float *x, 
                           int incx, float beta, float *y, int iny, float *WC, 
                           int kstan)
{
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
...
__global__ void
test_l_ssymv_special_update_v6_ts_tesla(int n, float alpha, float* A, int lda, float *x, 
                                  int inx, float beta, float *y, int iny, 
                                  float *WC, int kstan)
{
  int tx = threadIdx.x ;
...
__global__ void
test_l_ssymv_generic_v6_ts_tesla(int n, float alpha, float* A, int lda, float *x, 
                           int inx, float beta, float *y, int iny, float *WC,
...
__global__ void
test_l_ssymv_generic_update_v6_ts_tesla (int n, float alpha ,  float* A, int lda, float *x, int inx , float beta ,  float *y , int iny , float *WC, int kstan ){
  int tx = threadIdx.x ;
  int ind = (blockIdx.x)* 64 + tx ;
  float Ca = 0.f ;
  WC+= tx+(blockIdx.x)*64 + lda*blockIdx.x  ;
...
__global__ void
test_u_ssymv_generic_v6_ts_tesla (int n, float alpha ,  float* A, int lda, float *x, int inx , float beta ,  float *y , int iny , int m_full_block , int m_mod_32){
}
__global__ void
test_u_ssymv_special_v6_ts_tesla (int n, float alpha ,  float* A, int lda, float *x, int inx , float beta ,  float *y , int iny ){
}



extern "C"
void magmablas_ssymv6_tesla(char uplo, int m, float alpha, float *A, int lda, 
                      float *X, int incx, float beta, float *Y, int incy, 
                      float *dC_work, int kstan)
...
*/
    ...
    if(m % dgemv_bs == 0 ) {
       if( uplo == 'L' || uplo == 'l'){
	  test_l_ssymv_special_v6_ts_tesla <<<grid, threads>>>(m, alpha, 
                   A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
	  test_l_ssymv_special_update_v6_ts_tesla<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
       }
       else{
      ...
      if (uplo == 'L' || uplo == 'l'){
         test_l_ssymv_generic_v6_ts_tesla <<<grid, threads>>> (m, alpha, A, lda, 
                    X, incx ,beta,  Y , incy, dC_work, m_mod_64-1, kstan);
         test_l_ssymv_generic_update_v6_ts_tesla<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
      }	
      else{
         printf("Not Available Now.\n");
