...
__global__ void
l_dsymv_special (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny ){
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
...
__global__ void
l_dsymv_generic (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny , int m_full_block , int m_mod_32){
...
__global__ void
u_dsymv_generic (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny , int m_full_block , int m_mod_32){
...
__global__ void
u_dsymv_special (int n, double alpha ,  double* A, int lda, double *x, int inx , double beta ,  double *y , int iny ){
  int tx = threadIdx.x ; 
  int ty = threadIdx.y ; 
  int ind = blockIdx.x*  dgemv_bs + tx ;
  double res = 0.f;
...
*/
    ...
    if( m % dgemv_bs == 0 ) {
	    if( uplo == 'L' || uplo == 'l'){	
		    l_dsymv_special <<<grid, threads>>> (m, alpha , A, lda, X, incx ,beta,  Y , incy);
	    }
            else{
		    u_dsymv_special <<<grid, threads>>> (m, alpha , A, lda, X, incx ,beta,  Y , incy);
	    } 
	    ...
	    int  m_mod_32 = m%32 ;  
	    if( uplo == 'L' || uplo == 'l'){
		    l_dsymv_generic <<<grid, threads>>> (m, alpha , A, lda, X, incx ,beta,  Y , incy, m_full_block , m_mod_32);
	    }	
	    else{
		    u_dsymv_generic <<<grid, threads>>> (m, alpha , A, lda, X, incx ,beta,  Y , incy, m_full_block , m_mod_32);
	    }	
    }
}
...
extern "C" void  magmablas_dsymv (char uplo , int m , double alpha ,  double *A , int lda ,  double *X , int incx , double beta , double *Y , int incy )
{
/*
...
*/
        char side = 'a' ;
	mdsymv (side, uplo , m , alpha , A , lda , X , incx , beta , Y , incy );
