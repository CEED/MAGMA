...
//#define cublasDgemm magmablas_dgemm	// WARNING: causing ghost error
...
extern "C"
void b_copy_ (int M, int N, double *b, int ldb, double *d_x, int ldx)
{
	dim3 dimBlock((M>=MAX_THREAD_PER_BLOCK)?MAX_THREAD_PER_BLOCK:(WARP_SIZE*((M/WARP_SIZE)+(M%WARP_SIZE!=0))), 1);
	dim3 dimGrid(M/dimBlock.x+(M%dimBlock.x!=0), N);

	b_copy_kernel<<<dimGrid, dimBlock>>>(M, N, b, ldb, d_x, ldx);
	cudaThreadSynchronize();
}
...
void magmablas_dtrsm ( char side, char uplo, char tran, char diag, int M, int N, double alpha, double* A, int lda, double* b, int ldb)
{
	...
	if (side == 'l' || side == 'L')
	{
		...
		cudaMemset (d_dinvA, 9, NB*((M/NB)+(M%NB!=0))*NB*sizeof(double));
