...
__global__ void
magma_l_csymv_special_v6_ts_fermi(int n, float2 alpha,  float2* A, int lda, float2 *x, 
...
__global__ void
magma_l_csymv_special_update_v6_ts_fermi(int n, float2 alpha, float2* A, int lda, float2 *x, 
...
__global__ void
magma_l_csymv_generic_v6_ts_fermi(int n, float2 alpha, float2* A, int lda, float2 *x, 
...
__global__ void
magma_l_csymv_generic_update_v6_ts_fermi (int n, float2 alpha ,  float2* A, int lda, float2 *x, int inx , float2 beta ,  float2 *y , int iny , float2 *WC, int kstan ){
  int tx = threadIdx.x ;
  int ind = (blockIdx.x)* thread_x + tx ;
  float2 Ca;
  MAGMA_Z_SET2REAL(Ca, 0) ; 
...
extern "C"
void magmablas_csymv6_fermi(char uplo, int m, float2 alpha, float2 *A, int lda, 
...
*/
    ...
    if(m % dgemv_bs == 0 ) {
       if( uplo == 'L' || uplo == 'l'){
	  magma_l_csymv_special_v6_ts_fermi <<<grid, threads>>>(m, alpha, 
                   A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
	  magma_l_csymv_special_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
       }
       else{
      ...
      if (uplo == 'L' || uplo == 'l'){
         magma_l_csymv_generic_v6_ts_fermi <<<grid, threads>>> (m, alpha, A, lda, 
                    X, incx ,beta,  Y , incy, dC_work, m_mod_thread_x-1, kstan);
         magma_l_csymv_generic_update_v6_ts_fermi<<<grid, threads_u>>>(m, alpha, 
                        A, lda, X, incx ,beta,  Y , incy, dC_work, kstan);
      }	
      else{
         printf("Not Available Now.\n");
...
extern "C"
void  magmablas_csymv_fermi( char uplo , int m , float2 alpha,  float2 *A , int lda , 
				float2 *X , int incx, float2 beta, float2 *Y, int incy)
{
	...
	magmablas_csymv6_fermi(uplo, m, alpha, A, lda, X, incx, beta, Y, incy, 
                      dC_work, kstan);
