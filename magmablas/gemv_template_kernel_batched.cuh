    ...
    T beta, T**  y_array, int incy)
{
    ...
    gemvn_template_device<T, BLK_X, BLK_Y, TILE_SIZE>(m, n, alpha, A_array[batchid], lda, x_array[batchid], incx, beta, y_array[batchid], incy);
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    gemvn_kernel_batched<T, BLK_X, BLK_Y, TILE_SIZE><<< grid, threads, 0, queue >>>                    
    ...
    T beta, T**  y_array, int incy)
{
    ...
    gemvc_template_device<T, BLK_X, BLK_Y, TILE_SIZE, CONJA>(m, n, alpha, A_array[batchid], lda, x_array[batchid], incx, beta, y_array[batchid], incy);
}
    ...
    magma_int_t batchCount, magma_queue_t queue)
{
    ...
    if(CONJA == 1)
    {                         
        gemvc_kernel_batched<T, BLK_X, BLK_Y, TILE_SIZE, 1><<< grid, threads, 0, queue >>>                    
                ( m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy );        
    }
    else if(CONJA == 0)
    {
        gemvc_kernel_batched<T, BLK_X, BLK_Y, TILE_SIZE, 0><<< grid, threads, 0, queue >>>                    
                ( m, n, alpha, dA_array, ldda, dx_array, incx, beta, dy_array, incy );       
    }
}
