...
__global__ void zset_to_zero(double2 *A, int lda){
...
__global__ void zset_nbxnb_to_zero(int nb, double2 *A, int lda){
...
void zzero_32x32_block(double2 *A, int lda)
{
  // zset_to_zero<<<16, 32>>>(A, lda);
  zset_to_zero<<<32, 32>>>(A, lda);
}

void zzero_nbxnb_block(int nb, double2 *A, int lda)
{
