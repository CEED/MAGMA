...
__global__ void
l_dlat2s_generic (int n, double* A, int lda,  float *SA , int m_full_block , int m_mod_32 , int *INFO , double RMAX , int ldsa){
...
__global__ void
u_dlat2s_generic (int n, double* A, int lda, float *SA , int m_full_block , int m_mod_32 , int *INFO , double RMAX , int ldsa  ){
...
extern "C" void mdlat2s (char uplo , int m ,  double *A , int lda ,  float *Y , int LDSA , int *INFO )  
{
/*
Note:
	The UPLO = 'U' Version can be optimized more.
*/
    double RMAX = slamch_("O"); 
    int blocks;
    if (m % dgemv_bs==0)
        blocks = m/ dgemv_bs;
    else
...
extern "C" void 
magma_dlat2s ( char uplo , int n ,  double *A , int lda ,  float *SA , int LDSA ,  int *INFO )
{
