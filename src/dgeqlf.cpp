...
extern "C" int
magma_dgeqlf(int *m, int *n, double *a, int *lda, 
...
/*  -- MAGMA (version 0.2) --
	...
	*info = -4;
	...
	for (i = k - kk + ki; i >= k -kk; i -= nb) {
	    ...
	    if (i< k - kk + ki){
		 ...
		 This is the main update from the lookahead techniques. */
	      rows = *m - k + old_i + old_ib;
              ...
              magma_dlarfb('B', 'C', rows, cols, &old_ib,
	    ...
	    rows = *m - k + i + ib;
	    cols = *n - k + i;
	    dgeqlf_(&rows,&ib, a_ref(0,cols), lda, tau+i, work,lwork,&iinfo);

	    if (cols > 0) {
	        /* Form the triangular factor of the block reflector   
		   H = H(i+ib-1) . . . H(i+1) H(i) */
	        dlarft_("B", "C", &rows, &ib, a_ref(0, cols), lda, 
			tau + i, work, &ib);
       
		dpanel_to_q('L', ib, a_ref(rows-ib,cols), *lda, work+ib*ib);
		cublasSetMatrix(rows, ib, sizeof(double),
				a_ref(0,cols), *lda, da_ref(0,cols), ldda);
		dq_to_panel('L', ib, a_ref(rows-ib,cols), *lda, work+ib*ib);
    ...
    if (mu > 0 && nu > 0){
      ...
      dgeqlf_(&mu, &nu, a_ref(0,0), lda, tau, work, lwork, &iinfo);
    }
    work[0] = (double)*n * nb;
    return 0;
