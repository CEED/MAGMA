...
#define min(a,b)  (((a)<(b))?(a):(b))
    ...
    /* Function Body */
    *info = 0;
    int iinfo, nb = magma_get_dgetrf_nb(*m);
    ...
    if (nb <= 1 || nb >= min(*m,*n)) {
      /* Use CPU code. */
      cublasGetMatrix(*m, *n, sizeof(double), a, *lda, work, *lda);
      dgetrf_(m, n, work, lda, ipiv, info);
      cublasSetMatrix(*m, *n, sizeof(double), work, *lda, a, *lda);
    } else {
	...
	maxm = *m;
      ...
      double *W   = work;
      double *dAT = a;
      double *dA  = a + maxdim*maxdim;

      magmablas_dinplace_transpose( dAT, *lda, ldda );
      for( i = 0; i < s; i++ )
        {
	  // download i-th panel
	  cols = maxm - i*nb;
	  magmablas_dtranspose( dA, cols, inAT(i,i), ldda, nb, cols );
	  cublasGetMatrix( *m-i*nb, nb, sizeof(double), dA, cols, W, ldda); 
	  ...
	  // do the cpu part
	  rows = *m - i*nb;
	  dgetrf_( &rows, &nb, W, &ldda, ipiv+i*nb, &iinfo);
	  if (*info == 0 && iinfo > 0)
	    *info = iinfo + i*nb;
	  magmablas_dpermute_long( dAT, ldda, ipiv, nb, i*nb );

	  // upload i-th panel
	  cublasSetMatrix( *m-i*nb, nb, sizeof(double), W, ldda, dA, cols);
      ...
      rows = *m - s * nb;
      cols = *n - s * nb;
      cublasGetMatrix(rows, *n, sizeof(double), a+s*nb, *lda, work, rows);
      if (rows > 0)
	dgetrf_( &rows, &cols, work+s*nb*rows, &rows, ipiv+s*nb, &iinfo);
	...
	*info = iinfo + s*nb;
      ...
      int i2 = min(*m, *n) - s*nb;
      int one = 1, pp = s*nb;
      dlaswp_(&pp, work, &rows, &i1, &i2, ipiv+s*nb, &one);

      for(i=s*nb; i<min(*m, *n); i++)
	ipiv[i] += s*nb;
      cublasSetMatrix(rows, *n, sizeof(double), work, rows, a+s*nb, *lda);
    }
