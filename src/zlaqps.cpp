...
#define dF(i, j) (dF + (i) + (j)*(lddf))
    ...
    while( k < nb && lsticc == 0 ) {
        ...
        // Fortran BLAS idamax is 1-based; subtract 1 from k.
        // C       BLAS idamax is 0-based; do not subtract 1. */
        //pvt = k - 1 + idamax_(&i__1, &vn1[k], &ione);
        pvt = k + cblas_idamax( n - k + 1, &vn1[k], ione );
        
        if (pvt != k) {
            if (pvt <= nb){
                /* no need of transfer if pivot is within the panel */ 
                blasf77_zswap( &m, A(1, pvt), &ione, A(1, k), &ione );
            }
            else {
            ...
            i__1 = k - 1;
            blasf77_zswap( &i__1, F(pvt,1), &ldf, F(k,1), &ldf );
            itemp     = jpvt[pvt];
            jpvt[pvt] = jpvt[k];
            jpvt[k]   = itemp;
            vn1[pvt] = vn1[k];
       ...
       if (k < n) {
           i__1 = n - k;
           blasf77_zgemm( MagmaNoTransStr, MagmaConjTransStr, &ione, &i__1, &k,
                          &c_neg_one, A(rk, 1),     &lda,
                                      F(k + 1, 1),  &ldf,
                          &c_one,     A(rk, k + 1), &lda );
       }
    ...
    *kb = k;
    ...
    if (*kb < min(n, m - offset)) {
       ...
       magma_zgemm(MagmaNoTrans, MagmaConjTrans, i__1, i__2, *kb, c_neg_one,
                   dA(rk+1,1), ldda, &dF[*kb + 1 + ldf], i__2, c_one,
                   dA(rk+1, *kb + 1), ldda);
    }
