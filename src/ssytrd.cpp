...
{
  int one = 1, j;
  float mone = MAGMA_S_NEG_ONE, work[1];
  ...
  MAGMA_S_SET2REAL( res, slange_("f", &M, &N, ha, &M, work) );
	...
	for (i__ = *n - nb + 1; i__ >= i__1; i__ -= nb) 
	  {
	    ...
	    i__3 = i__ - 1;
	    ssyr2k_(uplo, "No transpose", &i__3, &nb, &c_neg_one, 
	...
	for (i__ = 1; i__ <= *n-nx; i__ += nb) 
	  {
	    ...
	    cublasSsyr2k('L', 'N', i__3, nb, c_neg_one, 
