...
#define min(a,b)  (((a)<(b))?(a):(b))
	...
	*info = -4;
	...
	cublasGetMatrix(m, n, sizeof(cuDoubleComplex), a, lda, work, m);
	lapackf77_zgetrf(&m, &n, work, &m, ipiv, info);
	cublasSetMatrix(m, n, sizeof(cuDoubleComplex), work, m, a, lda);
	free(work);
    }
    else {
	...
	cublasStatus status;
	status = cublasAlloc(nb*maxm, sizeof(cuDoubleComplex), (void**)&dA);
	...
	for( i=0; i<s; i++ )
            {
                ...
                cublasGetMatrix( m-i*nb, nb, sizeof(cuDoubleComplex), dA, cols, work, lddwork);
	...
	cublasGetMatrix(rows, nb0, sizeof(cuDoubleComplex), dA, cols, work, lddwork);
	...
	// upload i-th panel
	cublasSetMatrix(rows, nb0, sizeof(cuDoubleComplex), work, lddwork, dA, cols);
