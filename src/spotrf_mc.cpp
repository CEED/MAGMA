...
if (TRACE == 1) {
  ...
  //core_log_event_label(0x0000ff,QUARK_Thread_Rank(quark),0,QUARK_Get_Task_Label(quark));
...
cblas_ssyrk(CblasColMajor, CblasUpper, CblasTrans, N, K, mone, A1, LDA, one, 
  A2, LDA);

} else {

cblas_ssyrk(CblasColMajor, CblasLower, CblasNoTrans, N, K, mone, A1, LDA, one, 
  A2, LDA);
...
if (TRACE == 1) {
  core_event_end(QUARK_Thread_Rank(quark));
  //core_log_event_label(0x00ff00,QUARK_Thread_Rank(quark),0,QUARK_Get_Task_Label(quark));
...
if (TRACE == 1) {
  core_event_end(QUARK_Thread_Rank(quark));
  //core_log_event_label(0xffffff,QUARK_Thread_Rank(quark),0,QUARK_Get_Task_Label(quark));
      ...
      if (i < (*n)/2) {
        ...
        if (upper) {
          ...
          QUARK_Insert_Task(quark, SCHED_ssyrk, 0,
            ...
            sizeof(int),             &i,        VALUE,
            sizeof(float)*(*n)*(*n), A(0,i),    INPUT,
            sizeof(int),             lda,       VALUE,
            sizeof(float)*(*n)*(*n), A(i,i),    INOUT,
            sizeof(float)*(*n)*(*n), A(i-nb,i), INPUT,
          ...
          QUARK_Insert_Task(quark, SCHED_ssyrk, 0,
            ...
            sizeof(int),             &temp2,    VALUE,
            sizeof(int),             &i,        VALUE,
            sizeof(float)*(*n)*(*n), A(i,0),    INPUT,
            sizeof(int),             lda,       VALUE,
            sizeof(float)*(*n)*(*n), A(i,i),    INOUT,
            sizeof(float)*(*n)*(*n), A(i,i-nb), INPUT,
            strlen(label)+1,         label,     VALUE | TASKLABEL,
            6,                       "green",   VALUE | TASKCOLOR,
            0);
          ...
          for (k = 0; k < i; k += nb) {
            ...
            if (upper) {
              ...
              QUARK_Insert_Task(quark, SCHED_sgemm, 0,
                ...
                sizeof(int),             &nb,       VALUE,
                sizeof(float)*(*n)*(*n), A(k,i), INPUT,
                sizeof(int),             lda,       VALUE,
                sizeof(float)*(*n)*(*n), A(k,j),    INPUT,
                sizeof(float)*(*n)*(*n), A(i,j), INOUT,
                strlen(label)+1,         label,     VALUE | TASKLABEL,
              ...
              QUARK_Insert_Task(quark, SCHED_sgemm, 0,
                ...
                sizeof(int),             &nb,       VALUE,
                sizeof(int),             &nb,       VALUE,
                sizeof(float)*(*n)*(*n), A(j,k), INPUT,
                sizeof(int),             lda,       VALUE,
                sizeof(float)*(*n)*(*n), A(i,k),    INPUT,
                sizeof(float)*(*n)*(*n), A(j,i), INOUT,
                strlen(label)+1,         label,     VALUE | TASKLABEL,
                5,                       "blue",    VALUE | TASKCOLOR,
                0);
