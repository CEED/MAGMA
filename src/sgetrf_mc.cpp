...
cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 
  M, N, K, mone, A1, LDA, A2, LDA, fone, A3, LDA);
...
slaswp_(&N, A1, &LDA, &ione, &K2, IPIV, &ione); 

cblas_strsm(CblasColMajor, CblasLeft, CblasLower, CblasNoTrans, CblasUnit,
  K2, N, fone, A2, LDA, A1, LDA);

if (M > 0) {

cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 
  M, N, K, mone, A3, LDA, A1, LDA, fone, A4, LDA);
...
slaswp_(&N, A, &LDA, &ione, &K2, IPIV, &ione); 
  ...
  for (i = 0; i < k; i += nb) {
    ...
    for (j = 0; j < i; j += nb) { 
      ...
      QUARK_Insert_Task(quark, SCHED_panel_update, 0,
        sizeof(int),             &NN,      VALUE,
        sizeof(float)*(*m)*(*n), A(j,i),   INOUT,
        sizeof(int),             m,        VALUE,
        sizeof(int),             &MM,      VALUE,
        sizeof(float)*nb,        &ipiv[j], INPUT,
        sizeof(float)*(*m)*(*n), A(j,j),   INPUT,
        sizeof(int),             &MMM,     VALUE,
        sizeof(int),             &nb,      VALUE,
        sizeof(float)*(*m)*(*n), A(l,j),   INPUT,
        sizeof(float)*(*m)*(*n), A(l,i),   INOUT,
        sizeof(int),             &priority,VALUE | TASK_PRIORITY,
        sizeof(float)*(*m)*(*n), A(i,i),   OUTPUT,
      ...
      for (l = j + (2*nb); l < (*m); l += nb) {
        ...
        QUARK_Insert_Task(quark, SCHED_sgemm, 0,
          ...
          sizeof(float)*(*m)*(*n), A(l,j),   INPUT,
          sizeof(int),             m,        VALUE,
          sizeof(float)*(*m)*(*n), A(j,i),   INPUT,
          sizeof(float)*(*m)*(*n), A(l,i),   INOUT,
          sizeof(int),             &priority,VALUE | TASK_PRIORITY,
          sizeof(float)*(*m)*(*n), A(i,i),   OUTPUT | GATHERV,
    ...
    QUARK_Insert_Task(quark, SCHED_sgetrf, 0,
      sizeof(int),             &M,       VALUE,
      sizeof(int),             &N,       VALUE,
      sizeof(float)*(*m)*(*n), A(i,i),   INOUT,
      sizeof(int),             m,        VALUE,
      sizeof(float)*nb,        &ipiv[i], OUTPUT,
  ...
  for (i = 0; i < k; i += nb) {
    ...
    for (j = 0; j < i; j += nb) {
      ...
      QUARK_Insert_Task(quark, SCHED_slaswp, 0,
        sizeof(int),             &nb,       VALUE,
        sizeof(float)*(*m)*(*n), A(i,j),    INOUT,
        sizeof(int),             m,         VALUE,
        sizeof(int),             &MM,       VALUE,
        sizeof(float)*nb,        &ipiv[i],  INPUT,
        sizeof(int),             &priority, VALUE | TASK_PRIORITY,
        sizeof(void*),           fakedep,   INPUT,
        sizeof(float)*(*m)*(*n), A(i+nb,j), OUTPUT,
        strlen(label)+1,         label,     VALUE | TASKLABEL,
