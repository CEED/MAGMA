...
/*  -- MAGMA (version 1.1) --
    ...
    *info = 0;
    ...
    for (i = 0; i < k; ++i) {
        ...
        //double norm;
        //cublasGetVector(1, sizeof(double), dwork+i, 1, &norm, 1);
        //printf("norm %2d = %f\n", i, norm);

        magma_zlarfg_gpu(i__2, da_ref(i, i), da_ref(min(i__3,*m), i), dtau+i, dwork+i);
        ...
        if (i <= *n) {            
            ...
            /* The commented out lines have to be put in the GPU kernel */
            // alpha = *a_ref(i, i);
            // *a_ref(i, i) = MAGMA_Z_ONE;
            // z__1 = MAGMA_Z_CNJG( tau[i] );
            cublasGetVector(1, sizeof(cuDoubleComplex), da_ref(i, i), 1, &alpha, 1);
            magma_zlarf_gpu(i__2, i__3, da_ref(i, i), dtau+i, da_ref(i, i+1), *ldda,
                            dwork+i+1);
            cublasSetVector(1, sizeof(cuDoubleComplex), &alpha, 1, da_ref(i, i), 1);
