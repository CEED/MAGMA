...
/*  -- MAGMA (version 0.2) --
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= *n) {
      /*  Use unblocked code. */
      cublasGetMatrix(*n, *n, sizeof(double), a + a_offset, *lda, work, *n);
      dpotrf_(uplo, n, work, n, info);
      cublasSetMatrix(*n, *n, sizeof(double), work, *n, a + a_offset, *n);
    } else {
	...
	if (upper) {
		...
		jb = min(i__3,i__4);
		i__3 = j - 1;
                cublasDsyrk('u', 't', jb, i__3, c_b13, a_ref(1,j),
                             *lda, c_b14, a_ref(j, j), *lda);
                cudaMemcpy2DAsync(work, jb*sizeof(double), a_ref(j,j), 
				  (*lda) * sizeof(double), 4*sizeof(double), 
		...
		if (j + jb <= *n) {
		    ...
		    i__4 = j - 1;
                 ...
                 dpotrf_("Upper", &jb, work, &jb, info);
		 if (*info != 0) {
		   *info = *info + j - 1;
		   break;
		 }
                 cudaMemcpy2DAsync(a_ref(j,j), (*lda) * sizeof(double), work, 
				   jb*sizeof(double), sizeof(double)*jb, 
		...
		jb = min(i__3,i__4);
		i__3 = j - 1;
                cublasDsyrk('l', 'n', jb, i__3, c_b13, a_ref(j,1), 
                             *lda, c_b14, a_ref(j, j), *lda);
                cudaMemcpy2DAsync(work, jb*sizeof(double), a_ref(j,j), 
				  (*lda) * sizeof(double), sizeof(double)*jb, 
				  jb, cudaMemcpyDeviceToHost,stream[1]);
