        ...
        for (i = n-1; i >= n - nb; --i) {
            ...
            if (i > 0) {
                ...
                *A(i-1,i) = MAGMA_Z_MAKE( 1, 0 );
                ...
                magmablas_zhemv_sync(ngpu, i, work, W(0, iw), queues );
        ...
        for (i = 0; i < nb; ++i) {
            ...
            if (i < n-1) {
                ...
                *A(i+1,i) = MAGMA_Z_MAKE( 1, 0 );
                ...
                /* synchronize */
                magmablas_zhemv_sync(ngpu, i_n, work, W(i+1,i), queues );
#ifdef PROFILE_SYMV
...
/**
    Purpose
    -------
    ZHEMV_SYNC ccumulates the partial results generated by ZHEMV_MGPU
    that computes the matrix-vector multiply with a symmetric matrix A.
    ...
    @param[in] COMPLEX_16 array, dimension (n*ngpus)
    work    magmaDoubleComplex
            work[k*n:(k+1)*n-1] contain the partial result
            from k-th GPU (except for k=0, which is in W).

    @param[in,out] COMPLEX_16 array, dimension (n)
    W       magmaDoubleComplex
            On entry, it contains the partial result from GPU 0.
            On return, it contains the final result.

    @param[in] magma_queue_t
    queue   The execution queue used to send the partial results to CPU.
    ********************************************************************/
extern "C" magma_int_t
    ...
    magma_queue_t queues[][10] )
{
    ...
    for( id=1; id < ngpu; id++ ) {
        ...
        blasf77_zaxpy( &n, &c_one, &work[id*n], &ione, W, &ione );
    }
