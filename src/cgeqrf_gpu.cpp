...
/*  -- MAGMA (version 0.2) --
   ...
   *info = 0;
   ...
   if (nb >= nbmin && nb < k && nx < k) {
	...
	ib = min(k-i, nb);
	rows = *m -i;
	cudaMemcpy2DAsync(  work_ref(i), ldwork*sizeof(float2),
			    a_ref(i,i), (*lda)*sizeof(float2),
			    sizeof(float2)*rows, ib,
	...
	if (i>0){
	  ...
	  cudaMemcpy2DAsync(a_ref(old_i, old_i), (*lda) * sizeof(float2), 
			    work_ref(old_i), ldwork * sizeof(float2),
			    sizeof(float2)*old_ib, old_ib,
			    cudaMemcpyHostToDevice,stream[0]);
	}

	cudaStreamSynchronize(stream[1]);
	cgeqrf_(&rows, &ib, work_ref(i), &ldwork, tau+i, hwork, &lhwork, info);
	/* Form the triangular factor of the block reflector
	   H = H(i) H(i+1) . . . H(i+ib-1) */
	clarft_("F", "C", &rows, &ib, work_ref(i), &ldwork, tau+i, hwork, &ib);
	cpanel_to_q('U', ib, work_ref(i), ldwork, hwork+ib*ib); 
	cublasSetMatrix(rows, ib, sizeof(float2), 
			work_ref(i), ldwork, a_ref(i,i), *lda);
        cq_to_panel('U', ib, work_ref(i), ldwork, hwork+ib*ib);

	if (i + ib < *n) {
	  cublasSetMatrix(ib, ib, sizeof(float2), hwork, ib, dwork, lddwork);

	  if (i+nb < k-nx)
	    /* Apply H' to A(i:m,i+ib:i+2*ib) from the left */
	    magma_clarfb('F','C', rows, ib, &ib, a_ref(i,i), lda, dwork,
			 &lddwork, a_ref(i,i+ib), lda, dwork+ib, &lddwork);
	  else {
	    magma_clarfb('F','C', rows, *n-i-ib, &ib, a_ref(i,i), lda, dwork,
			 &lddwork, a_ref(i,i+ib), lda, dwork+ib, &lddwork);
	    cublasSetMatrix(ib, ib, sizeof(float2),
			    work_ref(i), ldwork, a_ref(i,i), *lda);
	  }
	  old_i = i;
	  old_ib = ib;
   ...
   if (i < k) {
      ib   = *n-i;
      rows = *m-i;
      cublasGetMatrix(rows, ib, sizeof(float2),
                      a_ref(i,i), *lda, work, rows);
      lhwork = *lwork - rows*ib;
      cgeqrf_(&rows, &ib, work, &rows, tau+i, work+ib*rows, &lhwork, info);
      cublasSetMatrix(rows, ib, sizeof(float2),
