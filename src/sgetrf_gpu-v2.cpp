...
#define min(a,b)  (((a)<(b))?(a):(b))
    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= min(*m,*n)) {
      ...
      /* generate swp to parallel swp indexes for parallel GPU swapping */
      int *newipiv  = (int*)malloc(min(*m, *n)*sizeof(int));
      swp2pswp(min(*m, *n), ipiv, newipiv);
      cudaMemcpy(dipiv, ipiv, min(*m,*n)*sizeof(int), cudaMemcpyHostToDevice);
      free(newipiv);
    } else {
	...
	*info = iinfo + s*nb;
	...
	ipiv[i] += s*nb;
      ...
      //	printf("%d\n", newipiv[k]);
      cudaMemcpy(dipiv,newipiv,min(*m,*n)*sizeof(int),cudaMemcpyHostToDevice);
      for(int k=0;k<min(*m, *n); k++)
        newipiv[k] = 0;;
