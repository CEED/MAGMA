...
int 
magma_dlahr2(int *n, int *k, int *nb, 
	     double *d_a, double *d_v, double *a, int *lda, 
...
/*  -- MAGMA auxiliary routine (version 0.1) --
	...
	if (i__ > 1) {
	  ...
	  i__2 = *n - *k + 1;
	  i__3 = i__ - 1;
	  dcopy_(&i__3, &a[*k+i__-1+a_dim1], lda, &t[*nb*t_dim1+1], &c__1);
	  dtrmv_("u","n","n",&i__3,&t[t_offset], ldt, &t[*nb*t_dim1+1], &c__1);
	  dgemv_("NO TRANSPOSE", &i__2, &i__3, &c_b4, &y[*k + y_dim1],
	  ...
	  i__2 = *n - *k - i__ + 1;
	  i__3 = i__ - 1;
	  dgemv_("T", &i__2, &i__3, &c_b5, &a[*k + i__ + a_dim1], lda, 
		 &a[*k+i__+i__*a_dim1], &c__1, &c_b5, &t[*nb*t_dim1+1], &c__1);

	  /* w := T'*w */
	  i__2 = i__ - 1;
	  dtrmv_("U","T","N",&i__2, &t[t_offset], ldt, &t[*nb*t_dim1+1],&c__1);
	  
	  /* b2 := b2 - V2*w */
	  i__2 = *n - *k - i__ + 1;
	  i__3 = i__ - 1;
	  dgemv_("N", &i__2, &i__3, &c_b4, &a[*k + i__ + a_dim1], lda, 
		 &t[*nb*t_dim1+1], &c__1, &c_b5, &a[*k+i__+i__*a_dim1], &c__1);

	  /* b1 := b1 - V1*w */
	  i__2 = i__ - 1;
	  dtrmv_("L","N","U",&i__2,&a[*k+1+a_dim1],lda,&t[*nb*t_dim1+1],&c__1);
