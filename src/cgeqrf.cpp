...
/*  -- MAGMA (version 0.1) --
   ...
   *info = 0;
   ...
   if (nb >= nbmin && nb < k && nx < k) {
      ...
      cudaMemcpy2DAsync(da_ref(0,nb),  ldda *sizeof(float2),
                         a_ref(0,nb), (*lda)*sizeof(float2),
                        sizeof(float2)*(*m), (*n-nb), 
                        cudaMemcpyHostToDevice,stream[0]);

      for (i = 0; i < k-nx; i += nb) {
	ib = min(k-i, nb);
	if (i>0){
	  cudaMemcpy2DAsync(  a_ref(i,i), (*lda)*sizeof(float2),
	...
	int rows = *m-i;
	cgeqrf_(&rows, &ib, a_ref(i,i), lda, tau+i, work, lwork, info);
   ...
   if (i < k) {
      ib = *n-i;
      if (i!=0)
	cublasGetMatrix(*m, ib, sizeof(float2),
