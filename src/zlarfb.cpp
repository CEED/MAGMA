...
#define dv_ref(a_1,a_2)    (dv+(a_2)*(*ldv) + a_1)
  ...
  double2 cone = {1.f,0.f}, czero = {0.f, 0.f}, cmone = {-1.f, 0.f};
  
  if (storev == 'c' || storev == 'C'){
    cublasZgemm('c', 'n', n, *k, m, cone, dc_ref(0, 0), *ldc,
		dv_ref(0,0), *ldv, czero, dwork, *ldwork);
    
    if (direct == 'F' || direct =='f')
      magmablas_ztrmm('r', 'u', 'n', 'n',
		      n, *k, cone, dt, *ldt, dwork, *ldwork);
    else
      magmablas_ztrmm('r', 'l', 'n', 'n',
		      n, *k, cone, dt, *ldt, dwork, *ldwork);
    
    cublasZgemm('n', 'c', m, n, *k, cmone, dv_ref(0, 0), *ldv,
		dwork, *ldwork, cone, dc_ref(0,0), *ldc);
  }
  else {
    cublasZgemm('n', 'c', m, *k, n, cone, dc_ref(0, 0), *ldc,
                dv_ref(0,0), *ldv, czero, dwork, *ldwork);
    
    magmablas_ztrmm('r', 'u', 'n', 'n',
		    m, *k, cone, dt, *ldt, dwork, *ldwork);
    
    cublasZgemm('n', 'n', m, n, *k, cmone, 
		dwork, *ldwork,
		dv_ref(0, 0), *ldv, 
		cone, dc_ref(0,0), *ldc);
