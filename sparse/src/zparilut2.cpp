    ...
    magma_queue_t queue )
{
    ...
    t_rm=0.0; t_add=0.0; t_res=0.0; t_sweep1=0.0; t_sweep2=0.0; t_cand=0.0;
     ...
     //   printf("Z = LU cup A0\n");
        ...
        // option 1 : candidate search
        start = magma_sync_wtime( queue );
        magma_zparilut_candidates( L0, U0, L, U, &hL, &hU, queue );
        end = magma_sync_wtime( queue ); t_cand=+end-start;
        
        start = magma_sync_wtime( queue );
        magma_zparilut_residuals_list( hA, L, UT, &hL, queue );
        magma_zparilut_residuals_list( hA, L, UT, &hU, queue );
        ...
        /*
        start = magma_sync_wtime( queue );
        CHECK( magma_z_spmm( MAGMA_Z_ONE, L, U, &LU, queue ) );
        CHECK( magma_zmatrix_cup( hL, hU, &LU, queue ) );
        magma_zmfree( &hL, queue );
        magma_zmfree( &hU, queue );
        end = magma_sync_wtime( queue ); t_cand=+end-start;
        start = magma_sync_wtime( queue );
        CHECK( magma_zmatrix_cup( hA, LU, &Z, queue ) );
        end = magma_sync_wtime( queue ); t_reorder1=+end-start;
        
        
        start = magma_sync_wtime( queue );
        magma_zparilut_residuals_list( hA, L, UT, &Z, queue );
        end = magma_sync_wtime( queue ); t_res=+end-start;
        
        start = magma_sync_wtime( queue );
        magma_zparilut_elementsum( Z, &sum, queue );
        printf("\n\n\n%%%sum:%.4e\n", sum );
        end = magma_sync_wtime( queue ); t_res+=end-start;
        
        start = magma_sync_wtime( queue );
        CHECK( magma_zmatrix_tril_negcap(  Z, L, &ZL, queue ) );
        CHECK( magma_zmatrix_triu_negcap( Z, U, &ZU, queue ) );
        end = magma_sync_wtime( queue ); t_reorder1=+end-start;
        
        start = magma_sync_wtime( queue );
        CHECK( magma_zparilut_transpose( ZU, &ZUT, queue) );
        end = magma_sync_wtime( queue ); t_rowmajor+=end-start;
        magma_zmfree( &ZU, queue );
        
        start = magma_sync_wtime( queue );
        magma_zparilut_selectoneperrow( &ZL, queue );
        magma_zparilut_selectoneperrow( &ZUT, queue );
        num_rmL = max(ZL.nnz - A.num_rows * ( precond->rtol ),0);
        num_rmU = max(ZUT.nnz - A.num_rows * ( precond->rtol ),0); 
        //magma_zparilut_set_thrs_randomselect( num_rmL, &ZL, 1, &thrsL, queue );
        //magma_zparilut_set_thrs_randomselect( num_rmU, &ZUT, 1, &thrsU, queue );
        #pragma omp parallel
        {
            magma_int_t id = omp_get_thread_num();
            if( id == 0 ){
        ...
        magma_zparilut_thrsrm( &ZL, &thrsL, queue );
        magma_zparilut_thrsrm( &ZUT, &thrsU, queue );
        end = magma_sync_wtime( queue ); t_select+=end-start;
        
        
        start = magma_sync_wtime( queue );
        CHECK( magma_zparilut_transpose( ZUT, &ZU, queue) );
        end = magma_sync_wtime( queue ); t_rowmajor+=end-start;
        magma_zmfree( &ZUT, queue );
        
        start = magma_sync_wtime( queue );
        CHECK( magma_zmatrix_cup(  L, ZL, &L_new, queue ) );
        CHECK( magma_zmatrix_cup(  U, ZU, &U_new, queue ) );
        end = magma_sync_wtime( queue ); t_reorder1=+end-start;
        */
        
        
        start = magma_sync_wtime( queue );
        CHECK( magma_zparilut_transpose( U_new, &UT, queue) );
        end = magma_sync_wtime( queue ); t_transpose1+=end-start;
        start = magma_sync_wtime( queue );
        CHECK( magma_zparilut_sweep( &A0, &L_new, &UT, queue ) );
        ...
        start = magma_sync_wtime( queue );
        magma_zmfree( &U, queue );
        CHECK( magma_zparilut_transpose( UT, &U, queue) );
        CHECK( magma_zmatrix_swap( &L_new, &L, queue) );
        magma_zmfree( &L_new, queue );
