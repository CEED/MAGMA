...
int main( int argc, char** argv)
{
    ...
    for( int itest = 0; itest < opts.ntest; ++itest ) {
        ...
        for( int iter = 0; iter < opts.niter; ++iter ) {
            ...
            TESTING_CHECK( magma_zmalloc_cpu( &VT,    ldv*N ));   // N x N (jobvt=A) or min(M,N) x N
            TESTING_CHECK( magma_zmalloc_cpu( &U,     ldu*N_U )); // M x M (jobu=A)  or M x min(M,N)
            TESTING_CHECK( magma_dmalloc_cpu( &S1,    min_mn ));
            TESTING_CHECK( magma_dmalloc_cpu( &S2,    min_mn ));
            TESTING_CHECK( magma_zmalloc_pinned( &h_R,    lda*N ));
            TESTING_CHECK( magma_zmalloc_pinned( &h_work, lwork ));
            
            #ifdef COMPLEX
            TESTING_CHECK( magma_dmalloc_cpu( &rwork, 5*min_mn ));
            #endif
