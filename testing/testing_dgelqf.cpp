...
int main( int argc, char** argv) 
{
    ...
    // TTT: what should be the size for dgelqf-v2 ?
    int lwork = 2*size[9]*magma_get_dgelqf_nb(size[9]);
	...
	h_A[j] = rand() / (double)RAND_MAX;
      ...
      // magma_dgelqf(&N, &N, h_R, &N, tau, h_work, &lwork, d_A, info);
      magma_dgelqf2(&N, &N, h_R, &N, tau, h_work, &lwork, d_A, info);
	 ...
	 =================================================================== */
      start = get_current_time();
      dgelqf_(&N, &N, h_A, &lda, tau, h_work, &lwork, info);
      end = get_current_time();
      if (info[0] < 0)  
	printf("Argument %d of dgelqf had an illegal value.\n", -info[0]);     
	 ...
	 =================================================================== */
      ...
      dqrt02(&N, &N, &N, h_A, h_R, hwork_Q, hwork_R, &N, tau, 
