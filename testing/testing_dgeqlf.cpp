...
int main( int argc, char** argv) 
{
    ...
    int lwork = size[9]*magma_get_dgeqlf_nb(size[9]);
	...
	h_A[j] = rand() / (double)RAND_MAX;
      ...
      magma_dgeqlf(&N, &N, h_R, &N, tau, h_work, &lwork, d_A, info);

      for(j=0; j<n2; j++)
        h_R[j] = h_A[j];    
      /*  
      for(j=0; j<N; j++)
        h_R[j+j*N] = h_A[j+j*N] = (h_A[j+j*N] + 1000.);
      */
      /* ====================================================================
	 ...
	 =================================================================== */
      start = get_current_time();
      dgeqlf_(&N, &N, h_A, &lda, tau, h_work, &lwork, info);
      end = get_current_time();
      if (info[0] < 0)  
	printf("Argument %d of dgeqlf had an illegal value.\n", -info[0]);     
