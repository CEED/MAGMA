...
extern "C" void cqrt02_(int *, int *, int *, float2 *, float2 *, float2 *,
			float2 *, int *, float2 *, float2 *,
			int *, float *, float *);
...
int main( int argc, char** argv) 
{
    ...
    int lwork = size[9]*magma_get_cgeqrf_nb(size[9]);
    status = cublasAlloc(n2+lwork, sizeof(float2), (void**)&d_A);
    if (status != CUBLAS_STATUS_SUCCESS) {
	...
	h_A[j].y = rand() / (float)RAND_MAX;
      ...
      magma_cgeqrf(&N, &N, h_R, &N, tau, h_work, &lwork, d_A, info);
	...
	h_R[j].y = h_A[j].y;
	 ...
	 =================================================================== */
      ...
      cqrt02_(&N, &N, &N, h_A, h_R, hwork_Q, hwork_R, &N, tau,
