...
int main( int argc, char** argv) 
{
    ...
    int nb = magma_get_dgehrd_nb(size[9]);
    int lwork = size[9]*nb;
    status = cublasAlloc(n2+2*lwork+nb*nb, sizeof(double), (void**)&d_A);
    if (status != CUBLAS_STATUS_SUCCESS) {
	...
	h_A[j] = rand() / (double)RAND_MAX;
	 ...
	 =================================================================== */
      start = get_current_time();
      dgehrd_(&N, &ione, &N, h_A, &lda, tau, h_work, &lwork, info);
      end = get_current_time();
      if (info[0] < 0)  
	printf("Argument %d of dgehrd had an illegal value.\n", -info[0]);
