...
extern"C" {
    ...
    /*magma_int_t magma_dsygvdx_2stage_m(magma_int_t nrgpu, magma_int_t itype, char jobz, char range, char uplo, magma_int_t n,
                                       double *a, magma_int_t lda, double *b, magma_int_t ldb,
                                       double vl, double vu, magma_int_t il, magma_int_t iu,
                                       magma_int_t *m, double *w, double *work, magma_int_t lwork,
                                       magma_int_t *iwork, magma_int_t liwork, magma_int_t *info);
...
//#define USE_MGPU
    ...
    if (argc != 1){
        for(i = 1; i<argc; i++){
            ...
            if (strcmp("-itype", argv[i])==0){
                itype = atoi(argv[++i]);
                if (itype>0 && itype <= 3){
                   printf("  testing_dsygvdx -itype %d\n\n", (int) itype);
                }
                else {
                   printf("\nUsage: \n");
                   printf("  testing_dsygvdx -itype %d\n\n", (int) itype);
            ...
            if (strcmp("-L", argv[i])==0){
              ...
              printf("  testing_dsygvdx -L");
            }
            if (strcmp("-U", argv[i])==0){
              uplo = (char*)MagmaUpperStr;
              printf("  testing_dsygvdx -U");
            }
        ...
        printf("\nUsage: \n");
        printf("  testing_dsygvdx -L/U -N %d -itype %d\n\n", 1024, 1);
    }
    ...
    magma_int_t lwork = magma_dbulge_get_lq2(N) + 1 + 6*N + 2*N*N;
    ...
    for(i=0; i<4; i++){
...
#ifdef USE_MGPU
        magma_dsygvdx_2stage_m(nrgpu, itype, jobz[0], range, uplo[0],
                               N, h_R, N, h_S, N, vl, vu, il, iu, &m1, w1,
                               h_work, lwork,
                               iwork, liwork,
                               &info);
#else
        magma_dsygvdx_2stage(itype, jobz[0], range, uplo[0],
                             N, h_R, N, h_S, N, vl, vu, il, iu, &m1, w1,
                             h_work, lwork,
                             iwork, liwork,
                             &info);
        ...
        if ( checkres ) {
          /* =====================================================================
             Check the results following the LAPACK's [ds]sygvdx routine.
          ...
          if (itype == 1){
            blasf77_dsymm("L", uplo, &N, &m1, &d_one, h_A, &N, h_R, &N, &d_zero, h_work, &N);
            for(int i=0; i<m1; ++i)
              blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
            blasf77_dsymm("L", uplo, &N, &m1, &d_neg_one, h_B, &N, h_R, &N, &d_one, h_work, &N);
            result[0] *= lapackf77_dlange("1", &N, &m1, h_work, &N, rwork)/N;
          }
          else if (itype == 2){
            blasf77_dsymm("L", uplo, &N, &m1, &d_one, h_B, &N, h_R, &N, &d_zero, h_work, &N);
            for(int i=0; i<m1; ++i)
              blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
            blasf77_dsymm("L", uplo, &N, &m1, &d_one, h_A, &N, h_work, &N, &d_neg_one, h_R, &N);
            ...
            for(int i=0; i<m1; ++i)
              blasf77_dscal(&N, &w1[i], &h_R[i*N], &ione);
            blasf77_dsymm("L", uplo, &N, &m1, &d_one, h_B, &N, h_work, &N, &d_neg_one, h_R, &N);
            result[0] *= lapackf77_dlange("1", &N, &m1, h_R, &N, rwork)/N;
          }
          ...
          magma_dsygvdx(itype, 'N', range, uplo[0],
