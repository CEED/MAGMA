 ...
 * verify_answer
 */
extern "C" void verify_answer (int m, int n, double *d_b1, double *d_b2, double *resid)
{
	long i;
	double * h_btmp1, *h_btmp2;

	h_btmp1 = (double*)malloc(n*m*sizeof(double));
	h_btmp2 = (double*)malloc(n*m*sizeof(double));

	cudaMemcpy (h_btmp1, d_b1, m*n*sizeof(double), cudaMemcpyDeviceToHost);
	cudaMemcpy (h_btmp2, d_b2, m*n*sizeof(double), cudaMemcpyDeviceToHost);
	
	for (i=0; i<n*m; i++)
		h_btmp1[i] -= h_btmp2[i];

	*resid = (double)dlange_ ("F", &m, &n, h_btmp1, &m, NULL)/(double)dlange_ ("F", &m, &n, h_btmp2, &m, NULL);
...
int main(int argc, char** argv)
{
	...
	printf ("magma_dtrsm testing\n");
	printf ("\nusage: \n\t./testing_dtrsm -m (matrix size) -r from step to -p SIDE, UPLO, TRANSA, DIAG \n");		
	printf ("for example:\n\t./testing_dtrsm -r 1024 1024 4096 -p L L N U\n\n");
	...
	if (side=='r' || side=='R')	//testing for side='R'
	{
		...
		for (m=from; m<=to; m+=step)
		{
			...
			cudaMalloc((void**)&d_b2, m*n*sizeof(double));
			cudaMemcpy (d_A, h_A, n2*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy (d_b1, h_b, m*n*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy (d_b2, h_b, m*n*sizeof(double), cudaMemcpyHostToDevice);

			//----- do cublas-----//
			start = get_current_time();
			cublasDtrsm (side, uplo, transa, diag, m, n, pone, d_A, lda, d_b1, ldb);
			...
			magmablas_dtrsm (side, uplo, transa, diag, m, n, pone, d_A, lda, d_b2, ldb);//			<----------------------------
	...
	else	//testing for side='L'
	{
		...
		for (n=from; n<=to; n+=step)
		{
			...
			srand (m*m);
			/* Allocate host memory for the matrices */
			h_A = (double*)malloc(m2 * sizeof(double));
			if (h_A == NULL)
			{	printf ("!!!! host memory allocation error in GetData\n"); exit(0);	}
			h_b = (double*)malloc(n * m * sizeof(double));
			if (h_b == NULL)
			...
			cudaMalloc((void**)&d_A, m2*sizeof(double));
			cudaMalloc((void**)&d_b1, m*n*sizeof(double));
			cudaMalloc((void**)&d_b2, m*n*sizeof(double));
			cudaMemcpy (d_A, h_A, m2*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy (d_b1, h_b, m*n*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy (d_b2, h_b, m*n*sizeof(double), cudaMemcpyHostToDevice);

			//----- do cublas-----//
			start = get_current_time();
			cublasDtrsm (side, uplo, transa, diag, m, n, pone, d_A, m, d_b1, m);
			end = get_current_time();
			double cublas_time = GetTimerValue(start,end) ;
			...
			magmablas_dtrsm (side, uplo, transa, diag, m, n, pone, d_A, m, d_b2, m);//			<----------------------------
