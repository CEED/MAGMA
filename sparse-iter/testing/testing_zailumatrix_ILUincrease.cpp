...
int main( int argc, char** argv)
{
    ...
    real_Double_t t_cusparse, t_chow, t_recursive = 0.0;

    magma_z_sparse_matrix hA, hALCOO, hAUCOO, hAcusparse, hAtmp, dA, hLU, dL, dU, hL, hU, hUT;
    ...
    for(int matrix=1; matrix<2; matrix=matrix+1){
...
*/
    ...
    magma_z_sparse_matrix hACSRCOO, hAL, hAU, hAinitguess, dAinitguess, hAcopy;



    magma_z_mtransfer( hA, &hAtmp, Magma_CPU, Magma_CPU );
    ...
    //ILU-m levels
    for( int levels = 0; levels < 1000; levels++){ //ILU-m levels
    magma_z_mtransfer( hA, &hAcopy, Magma_CPU, Magma_CPU );
    magma_zsymbilu( &hAcopy, levels, &hL, &hUT ); 
    magma_zinitrecursiveLU( hA, &hAcopy );
    ...
    magma_zinitrecursiveLU( hAtmp, &hL );
    magma_zinitrecursiveLU( hAtmp, &hUT );
    // transpose U for the algorithm
    magma_z_cucsrtranspose(  hUT, &hU );
    magma_z_mfree( &hUT );
...
*/
  ...
  //  magma_zmreorder( hACSRCOO, n, blocksize, blocksize, blocksize, &hAinitguess );
    ...
    for(int z=0; z<numavg; z++){
        ...
        magma_z_mtransfer( hL, &dL, Magma_CPU, Magma_DEV );
        magma_z_mtransfer( hU, &dU, Magma_CPU, Magma_DEV );
        ...
        magma_device_sync(); end = magma_wtime();
        t_chow =t_recursive + end-start;
        if( iters == 2 )
            t_recursive += end-start;

        // check the residuals
        magma_z_mtransfer( dL, &hAL, Magma_DEV, Magma_CPU );
        magma_z_mtransfer( dU, &hUT, Magma_DEV, Magma_CPU );
        magma_z_cucsrtranspose(  hUT, &hAU );
        magma_z_mfree(&hUT);
        ...
        magma_z_mfree(&hAtmp);
        magma_zmlumerge( hAL, hAU, &hAtmp);

        // frobenius norm of error
        //magma_zfrobenius( hAcusparse, hAtmp, &res );

        // ilu residual
        magma_zilures(   hA, hAL, hAU, &hLU, &ilures ); 
