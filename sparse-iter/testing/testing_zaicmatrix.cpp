...
int main( int argc, char** argv)
{
    ...
    magma_z_mfree(&hAUt);
    magma_z_mconvert( hAcopy, &hAtmp, Magma_CSR, Magma_CSRL );

    printf("\n#================================================================================#\n");
    printf("# IC-(m)  #nnz  iters  blocksize  t_chow   t_cusparse     error        ICres \n");
    ...
    for(int z=0; z<numavg; z++){
        ...
        magma_z_cucsrtranspose(  hL, &hUT );

        // frobenius norm of error
        magma_zfrobenius( hAcusparse, hL, &res );
