...
//            "/home/hanzt/sparse_matrices/mtx/Trefethen_20000.mtx", //     n:20000 nnz:554466 nnz/n:27 max_nnz_row:29 
    ...
    magma_z_sparse_matrix hA, hAL, hACOO, hCOO, hAU, hAUT, dACOO, dCOO, hAcusparse, hA3U, hAtmp, dA, hAD, hADD, dAD, dADD, hLU, dAL, dAU, dL, dU, hL, hU, hAt, hUT, hUTCOO;
...
// matrix from UFMC
    ...
    // need CSRCOO
    magma_z_mconvert( hA, &hACOO, Magma_CSR, Magma_CSRCOO );

    magma_z_mtransfer( hACOO, &dACOO, Magma_CPU, Magma_DEV );
    ...
    magma_z_mtransfer( hACOO, &dCOO, Magma_CPU, Magma_DEV );


    magma_device_sync(); start = magma_wtime(); 

    for(int i=0; i<iters; i++){
        magma_zailu_csr_o( dACOO, dCOO );
    }

    magma_device_sync(); end = magma_wtime();
    t_chow = end-start;

    magma_z_mtransfer( dCOO, &hCOO, Magma_DEV, Magma_CPU );
    ...
    magma_zfrobenius( hCOO, hAcusparse, &res );

   // need only lower triangular
    hAL.diagorder_type == Magma_UNITY;
    magma_z_mconvert( hCOO, &hAL, Magma_CSR, Magma_CSRL );

    // need only upper triangular
    magma_z_mconvert( hCOO, &hAU, Magma_CSR, Magma_CSRU );
    magma_z_cucsrtranspose(  hAU, &hAUT );

    magma_znonlinres(   hA, hAL, hAUT, &hLU, &nonlinres ); 
