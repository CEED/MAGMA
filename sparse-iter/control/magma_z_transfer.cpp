    ...
    magma_queue_t queue )
{
    ...
    if ( src == Magma_CPU && dst == Magma_DEV ) {
        ...
        if ( A.storage_type == Magma_CSRCOO ) {
            ...
            *B = A;
            ...
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->drowidx, A.nnz );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_zsetvector( A.nnz, A.val, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_ELLPACK ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat = magma_zmalloc( &B->dval, A.num_rows * A.max_nnz_row );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->dcol, A.num_rows * A.max_nnz_row );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
        ...
        if ( A.storage_type == Magma_ELLDD ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat = magma_zmalloc( &B->dval, 2*A.num_rows * A.max_nnz_row );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->dcol, 2*A.num_rows * A.max_nnz_row );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_zsetvector( 2 * A.num_rows * A.max_nnz_row, A.val, 1, B->dval, 1 );
            magma_index_setvector( 2 * A.num_rows * A.max_nnz_row, A.col, 1, B->dcol, 1 );
        }
        //ELLRT-type
        if ( A.storage_type == Magma_ELLRT ) {
            ...
            // memory allocation
            stat = magma_zmalloc( &B->dval, A.num_rows * rowlength );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->dcol, A.num_rows * rowlength );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->drow, A.num_rows );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
        ...
        if ( A.storage_type == Magma_SELLC || A.storage_type == Magma_SELLP ) {
            ...
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->drow, A.numblocks + 1 );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_zsetvector( A.nnz, A.val, 1, B->dval, 1 );
        ...
        if ( A.storage_type == Magma_BCSR ) {
            ...
            // memory allocation
            stat = magma_zmalloc( &B->dval, size_b*size_b*A.numblocks );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->drow, r_blocks + 1 );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            stat = magma_index_malloc( &B->dcol, A.numblocks );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
        ...
        if ( A.storage_type == Magma_DENSE ) {
            ...
            B->diameter = A.diameter;
            // memory allocation
            stat = magma_zmalloc( &B->dval, A.num_rows*A.num_cols );
            if ( stat != 0 )
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
    ...
    if ( src == Magma_DEV && dst == Magma_CPU ) {
        ...
        if ( A.storage_type == Magma_ELLDD ) {
            ...
            magma_index_malloc_cpu( &B->col, 2*A.num_rows*A.max_nnz_row );
            // data transfer
            magma_zgetvector( 2*A.num_rows*A.max_nnz_row, A.dval, 1, B->val, 1 );
            magma_index_getvector( 2*A.num_rows*A.max_nnz_row, A.dcol, 1, B->col, 1 );
        }
        //ELLRT-type
        if ( A.storage_type == Magma_ELLRT ) {
            // fill in information for B
    ...
    if ( src == Magma_DEV && dst == Magma_DEV ) {
        ...
        if ( A.storage_type == Magma_ELLDD ) {
            ...
            {printf("Memory Allocation Error transferring matrix\n"); exit(0); }
            // data transfer
            magma_zcopyvector( 2 * A.num_rows*A.max_nnz_row, A.dval, 1, B->dval, 1 );
            magma_index_copyvector( 2 * A.num_rows*A.max_nnz_row, A.dcol, 1, B->dcol, 1 );
        }
        //ELLRT-type
        if ( A.storage_type == Magma_ELLRT ) {
            // fill in information for B
    ...
    magma_queue_t queue )
{
    ...
    if ( src == Magma_CPU && dst == Magma_DEV ) {
        ...
        y->major = x.major;
        // memory allocation
        stat = magma_zmalloc( &y->dval, x.num_rows );
        if ( stat != 0 )
