...
extern "C" magma_int_t
magma_zmdynamicic_candidates_ll(
    magma_z_matrix LU,
    magma_z_matrix *LU_new,
    magma_queue_t queue )
{
    ...
    for( magma_index_t row=0; row<LU.num_rows; row++){
        ...
        magma_index_t element1 = start;
        // loop first element over row - only for elements smaller the diagonal
        while( LU.list[element1] != 0 ) {


            magma_index_t element2 = LU.row[ LU.col[ element1 ] ];
            // loop second element over row
            while( LU.list[ element2 ] != 0 ) {
                // check whether the candidate already is included in LU
                magma_int_t exist = 0;
                magma_index_t checkrow = row; 
                magma_index_t checkcol = LU.col[ element2 ]; 
                magma_index_t check = LU.row[ row ];
                magma_index_t checkelement =  LU.col[check];
        ...
        while( LU.list[element1] != 0 ) {
            ...
            while( LU.list[ element2 ] != 0 ) {
                ...
                if( exist == 0 ){
                    ...
                    LU_new->col[ numadd[row] + ladd ] = checkcol;
                    ladd++;
                    //printf("---------------->>> add  candidate %d at (%d, %d)\n", ladd, row, checkcol);
