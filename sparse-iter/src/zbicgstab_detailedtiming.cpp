...
#define RTOLERANCE     10e-100
#define ATOLERANCE     10e-100
...
magma_int_t
magma_zbicgstab_detailedtiming( magma_z_sparse_matrix A, magma_z_vector b, magma_z_vector *x,  
    ...
    real_Double_t t_spmv=0., t_dot=0., t_pupdate=0., t_supdate=0., t_xrupdate=0.;

    real_Double_t t_s_spmv=0., t_s_dot=0., t_s_pupdate=0., t_s_supdate=0., t_s_xrupdate=0.;
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
                                                    solver_par->numiter++ ){
    magma_device_sync(); t_s_dot=magma_wtime();
        rho_new = magma_zdotc( dofs, rr.val, 1, r.val, 1 );  // rho=<rr,r>
    magma_device_sync(); t_dot+=magma_wtime()-t_s_dot;

    magma_device_sync(); t_s_pupdate=magma_wtime();
    ...
    magma_device_sync(); t_pupdate+=magma_wtime()-t_s_pupdate;

    magma_device_sync(); t_s_spmv=magma_wtime();
        magma_z_spmv( c_one, A, p, c_zero, v );              // v = Ap
    magma_device_sync(); t_spmv+=magma_wtime()-t_s_spmv;

    magma_device_sync(); t_s_dot=magma_wtime();
        alpha = rho_new / magma_zdotc( dofs, rr.val, 1, v.val, 1 );
    magma_device_sync(); t_dot+=magma_wtime()-t_s_dot;

    magma_device_sync(); t_s_supdate=magma_wtime();
        magma_zcopy( dofs, r.val, 1 , s.val, 1 );            // s=r
        magma_zaxpy( dofs, c_mone * alpha, v.val, 1 , s.val, 1 ); // s=s-alpha*v
    magma_device_sync(); t_supdate+=magma_wtime()-t_s_supdate;
    ...
    magma_device_sync(); t_s_xrupdate=magma_wtime();
    ...
    magma_device_sync(); t_xrupdate+=magma_wtime()-t_s_xrupdate;
    ...
    printf("# SpMV: %.2f %.2f\n", t_spmv, t_spmv/solver_par->runtime);
    printf("# dot: %.2f %.2f\n", t_dot, t_dot/solver_par->runtime);
    printf("# p_update: %.2f %.2f\n", t_pupdate, t_pupdate/solver_par->runtime);    
    printf("# s_update: %.2f %.2f\n", t_supdate, t_supdate/solver_par->runtime);
    printf("# xr_update: %.2f %.2f\n", t_xrupdate, t_xrupdate/solver_par->runtime);
    printf("#=========================================================='\n");
