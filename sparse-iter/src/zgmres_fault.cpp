...
magma_int_t
magma_zgmres_fault( magma_z_sparse_matrix A, magma_z_vector b, magma_z_vector *x,  
    ...
    for( solver_par->numiter= 1; solver_par->numiter<solver_par->maxiter; 
        ...
        for(k=1; k<=restart; k++) {
            ...
            if (solver_par->ortho == Magma_MGS ) {
                ...
                // perturb this entry every f-th loop
                if( k%f1 == 0 && solver_par->numiter%f2 == 0 )
                    H(k+1,k) = H(k+1,k) * perturbation;
