...
/*  -- MAGMA (version 1.1) --
    ...
    This is a GPU implementation of the PGMRES method.
    ...
    magma_precond_parameters *precond_par     precond parameters 

    =====================================================================  */

magma_int_t
magma_zpgmres( magma_z_sparse_matrix A, 
    ...
    magma_z_vector z, z_t;
    magma_z_vinit( &z, Magma_DEV, dofs*((solver_par->restart)+1), c_zero );
    ...
    for (iter = 0; iter<solver_par->maxiter; iter++) {
        for(k=1; k<=(solver_par->restart); k++) {
            ...
            magma_zcopy(dofs, z_t.val, 1, z.val+k*dofs, 1);   //z[k] = z_t
            magma_z_spmv( c_one, A, z_t, c_zero, r ); //  r       = A z 
        ...
        //   Minimization Done      
        for (i=1; i<=m; i++)
            magma_zaxpy(dofs, y[i], z.val+i*dofs, 1, x->val, 1);  //  xNew += y[i]*q[i]
